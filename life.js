...
function home() {
 ...
 const doneWishes=state.wishes.filter(w=>w.state==='已完成').length;
 const countType=t=>state.records.filter(r=>r.type===t).length;
 const modules=[['wishes','wish','人生清单',`${doneWishes}/${state.wishes.length}`],['places','place','去过的地方',`${distinct('city')}`],['first','first','1000个第一次',`${countFirst()}`],['food','food','美食记录',`${countType('food')}`],['movie','movie','电影记录',`${countType('movie')}`],['numbers','numbers','人生数字',`${Stats.length} 大类`]];
 return `<header class="app-header">${brand()}<div class="header-tools"><button class="icon-button" data-go="search" aria-label="搜索记录">${icon('search')}</button><button class="icon-button" data-go="timeline" aria-label="查看时间轴">${icon('bell')}</button></div></header><button class="life-banner" data-go="numbers"><div class="banner-copy"><span>${days?'已经和这个世界相遇了':'已经收藏了'}</span><div><strong>${(days || state.records.length).toLocaleString()}</strong> ${days?'天':'个瞬间'}</div><p>继续出发，去体验更多可能。</p></div></button><section class="modules" aria-label="人生入口">${modules.map(([p,i,t,n])=>`<button class="module ${i}" data-go="${p}"><span class="module-icon">${icon(i)}</span><strong>${t}</strong><small>${n}</small></button>`).join('')}</section>${sectionHead('最近的人生','timeline')}${recordList(sorted().slice(0,3))}<p class="tiny-note">${state.demo?'正在体验示例人生 · 你添加的记录保存在本机':'记录保存在当前浏览器'} · 默认私密</p>`;
}
