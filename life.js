...
let coverTimer=null;
function go(p) { if(p==='home'&&page==='cover'){clearTimeout(coverTimer);const loader=document.querySelector('.cover-loading');loader?.classList.add('is-visible');const wait=matchMedia('(prefers-reduced-motion:reduce)').matches?220:2800;coverTimer=setTimeout(()=>{page=p;render();window.scrollTo(0,0);},wait);return;} page=p; render(); window.scrollTo(0,0); }
