...
openForm = function(type,wishId='',recordId=''){
 ...
 <div class="field mood-field">心情<span class="mood-picker">${moods.map(([k,e])=>`<button type="button" class="mood ${r.mood===k?'picked':''}" data-mood="${k}" aria-pressed="${r.mood===k?'true':'false'}" aria-label="心情：${k}">${e}</button>`).join('')}</span><input type="hidden" name="mood" value="${esc(r.mood || '')}"/></div>
...
document.addEventListener('click',event=>{
 ...
 if(d.mood&&$('#experience-record')){handled();const form=$('#experience-record');form.querySelector('[name="mood"]').value=d.mood;form.querySelectorAll('[data-mood]').forEach(b=>{const on=b.dataset.mood===d.mood;b.classList.toggle('picked',on);b.setAttribute('aria-pressed',String(on));});$('#discard-confirm')?.remove();return;}
 if(d.ex){handled();
...
recordList = function(items){
 ...
 return `<div class="record-list${page==='food'?' grid-2':page==='movie'?' grid-3':''}">${items.map(r=>`<article class="record${page==='first'?' record-stack':''}" data-kind="${esc(r.type)}"><button class="record-hit" data-detail="${esc(r.id)}">${picture(r)}<div class="record-copy"><h3>${esc(r.title)}</h3><p>${shortDate(r.date)}${r.city?' · '+esc(r.city):''}</p>${Number(r.rating)>0?`<p class="rate-line" aria-label="${Number(r.rating)} 分">${icon('wish')} ${Number(r.rating).toFixed(1)}</p>`:''}${['home','timeline','search','favorites','places','first'].includes(page)&&r.note?`<small>${esc(r.note)}</small>`:''}</div></button><button class="fav-button ${r.favorite?'on':''}" data-favorite="${esc(r.id)}" aria-label="${r.favorite?'取消收藏':'收藏这个瞬间'}" aria-pressed="${r.favorite?'true':'false'}">${icon('heart')}</button></article>`).join('')}</div>`;
};
details = function(){
 ...
 return `${heading('一个人生瞬间')}${media.length?`<div class="detail-gallery">${media.map((m,i)=>`<button data-ex="preview" data-id="${esc(r.id)}" data-index="${i}" aria-label="预览第 ${i+1} 张照片"><img src="${m.src}" alt="${esc(r.title)} · 照片 ${i+1}" loading="lazy"/></button>`).join('')}</div>`:picture(r,'detail-photo')}<div class="detail-body"><span class="tag">${esc(labels[r.type]||'生活瞬间')}${r.first?' · 第一次':''}</span><h2>${esc(r.title)}</h2><p class="detail-meta">${shortDate(r.date)} · ${esc([r.city,r.place].filter(Boolean).join(' · ')||'地点未填写')}</p>${r.mood?`<p class="detail-meta">心情 · ${esc(r.mood)}</p>`:''}${r.companion?`<p class="detail-meta">和 ${esc(r.companion)} 一起</p>`:''}${r.rating?`<p class="rating-display" aria-label="评分">${Math.min(5,Math.max(0,Number(r.rating)||0))} / 5</p>`:''}<p class="detail-note">${esc(r.note||'那天的感受，留待下次慢慢补上。')}</p>${wish?`<p class="detail-meta">来自心愿：${esc(wish.title)}</p>`:''}${(Array.isArray(r.tags)?r.tags:[]).map(tag=>`<span class="tag">${esc(tag)}</span>`).join(' ')}<div class="detail-actions"><button class="pill" data-ex="edit-record" data-id="${esc(r.id)}">编辑记录</button><button class="pill" data-favorite="${esc(r.id)}">${icon('heart')} ${r.favorite?'已收藏':'收藏'}</button><button class="text-button danger" data-delete="${esc(r.id)}">删除记录</button></div></div>`;
};
const wishBuckets=[['全部',()=>true],['想去',w=>w.category==='想去'],['想做',w=>['想做','想学'].includes(w.category)],['想体验',w=>['想体验','想吃','想看','其他'].includes(w.category)]];
wishesPage = function(){
 const match=wishBuckets.find(b=>b[0]===wishFilter),items=state.wishes.filter(w=>(match?match[1]:()=>true)(w));
 return `${heading('人生清单')}<div class="chips">${wishBuckets.map(([label])=>`<button class="chip ${wishFilter===label?'selected':''}" data-wish-filter="${label}">${label}</button>`).join('')}</div><div class="wish-list">${items.map(w=>{const related=state.records.find(r=>r.wishId===w.id);return `<article class="wish-row"><div class="wish-main">${picture(w)}<div><h3>${esc(w.title)}</h3><small>${esc(w.category)} · ${esc(w.state)}${w.completedAt?' · '+shortDate(w.completedAt):''}</small></div>${icon('arrow','wish-chevron')}</div><div class="wish-tools"><button class="text-button" data-ex="edit-wish" data-id="${esc(w.id)}">编辑</button><button class="text-button danger" data-ex="delete-wish" data-id="${esc(w.id)}">删除</button>${related?`<button class="pill" data-detail="${esc(related.id)}">查看记录</button>`:`<button class="pill" data-complete="${esc(w.id)}">完成并记录</button>`}</div></article>`;}).join('')||'<div class="empty"><h3>把想做的事，先写下来。</h3><p>不着急完成，有向往就很好。</p></div>'}</div><button class="primary full" data-action="wish">${icon('plus')} 添加心愿</button>`;
};
...
// placesPage 已在 life.js 内含「世界 / 中国 / 城市 / 列表」范围切换与最近去过列表。
